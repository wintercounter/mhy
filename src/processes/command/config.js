...
const handleWrite = (config, dir, format, overwrite) => {
    ...
    for (const writeConfig of modules) {
        process.stdout.write(
            `\r Writing configs ${results.length + 1}/${modules.length}`
        )
    ...
    const cleanlyInitialized = results.filter(
        ({ isExisted, isOverwritten }) => !isOverwritten && !isExisted
    )

    if (cleanlyInitialized.length || overwritten.length) {
        console.log(
            `\nInitialized ${cleanlyInitialized.length +
                overwritten.length} config(s) successfully:\n`
        )
        for (const { filename, isOverwritten } of [
            ...cleanlyInitialized,
            ...overwritten
        ]) {
            console.log(
                `    ${chalk.green(figures.tick)} ${filename}${
                    isOverwritten
                        ? ` ${chalk.yellow(`${figures.warning} overwritten`)}`
                        : ''
                }`
            )
        }
    ...
    if (!overwrite && existed.length) {
        console.log('\n')
        process.stdout.write(
            `\rSkipped ${existed.length} already existing config(s):\n`
        )
...
const fetchConfigs = (config = []) => {
    ...
    return glob.sync(
        path.resolve(__dirname, '../../configs/', globStr) + path.sep
    )
}

const fetchWriters = (config = []) => {
    const glob = require('glob')
    const globStr = config.length
        ? `*(${config.join('|')})/write.js`
        : '*/write.js'
